...
import * as Image from '../assets/image.svg';


const QuestionScreen = () => {

  //PROCESS 1 => WITH TIMER
  //PROCESS 2 => WITH NEXT CLICK

  const [count, setCount] = useState(1);
  const [nextClicked, setNextClicked] = useState(false);
  const [question, setQuestion] = useState('');
  const [totalQuesLength, setTotalQuesLength] = useState(0);

  const fetchQuestion = async () => {
    console.log("fetchQuestion is called")

    if (totalQuesLength && count > totalQuesLength) {
      //show score card
    }
    else {
      const res = await fetch(`http://localhost:3000/questions/${count}`);
  ...
  const fetchData = async () => {
    ...
    setTotalQuesLength(data.length);
  }
  ...
  useEffect(() => {
    ...
    return () => {
      console.log("return func in useffect")
      //clear the timer again if next is clicked
      clearInterval(timeout);
      // fetchQuestion()
    }
  }, [count])
    ...
    <div className='background'>
      <div className="confetti">
        <svg width="750" height="157" viewBox="0 0 750 157" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <path fillRule="evenodd" clipRule="evenodd" d="M154.818 40C182.432 40 204.818 17.6142 204.818 -10C204.818 -37.6142 182.432 -60 154.818 -60C127.204 -60 104.818 -37.6142 104.818 -10C104.818 17.6142 127.204 40 154.818 40Z" fill="#37DAFF" />
          <rect x="599.622" y="-39.6274" width="80" height="144" transform="rotate(-45 599.622 -39.6274)" fill="#FFE949" />
          <rect x="465.896" y="91.2203" width="8" height="15" transform="rotate(15 465.896 91.2203)" fill="#FFE949" />
          <rect x="317.077" y="13.2203" width="8" height="15" transform="rotate(15 317.077 13.2203)" fill="#E0B6FF" />
          <rect x="126.104" y="74.0048" width="8" height="15" transform="rotate(30 126.104 74.0048)" fill="#FFE949" />
          <rect width="8" height="15" transform="matrix(-0.707107 0.707107 0.707107 0.707107 594.343 113.368)" fill="#FF3C00" />
          <rect width="12" height="20" transform="matrix(-0.707107 0.707107 0.707107 0.707107 9.17163 9.53986)" fill="#FFA746" />
          <rect width="16" height="30" transform="matrix(-0.5 0.866025 0.866025 0.5 581.828 3.57181)" fill="#49FFF4" />
          <rect x="687.148" y="137.768" width="10" height="20" transform="rotate(60 687.148 137.768)" fill="#49FFF4" />
        </svg>
      </div>
      <div className='questions_bg'>
        <div className='numbering_circle'>
          <div className='inner_numbering_circle'>
            <div className='numbering'>
              <span className='bigger_number'>{question.id}</span>
              <span className='smaller_number'>/{totalQuesLength}</span>
            </div>
          </div>
        </div>
        ...
        {/* <div className='question_image'>
          <img src={Image.default} alt='mySvgImage' />
        </div> */}
        ...
        <div>
          <Button variant="contained"
            //on next click we need to increase count value and start the timer again
            onClick={() => { setCount(count + 1); setNextClicked(true); }}
            sx={{
              width: '90vw',
              marginBlockEnd: '1em',
              height: '4em',
